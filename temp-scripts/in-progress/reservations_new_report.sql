...
SELECT C.id as CaseId
    ...
    ,Products.ProductId as HimaliaProductId
...
inner join (
  ...
  left join Himalia.meta.Risks as R on R.gid = Cl.RiskGID
  inner join (
  select
...
left join (
  ...
  inner join Himalia.dbo.Products as PO on PO.gid = P.gid and PO.Deleted = 0
  inner join Himalia.dbo.Programs as Pg on Pg.ProductGID = PO.gid and Pg.Deleted = 0
  inner join Himalia.meta.ProgramTypes as Pt on Pt.gid = Pg.ProgramTypeGID
  inner join Himalia.meta.InsuranceTypes as IT on IT.gid = Pt.InsuranceTypeGID
  inner join Himalia.dbo.Branches as B on B.gid = PO.BranchGID
  inner join Himalia.dbo.InsuranceObjects as IObj on IObj.ProgramGID = Pg.gid
